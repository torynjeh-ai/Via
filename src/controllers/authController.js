  ...
  async (req, res, next) => {
    try {
      ...
      res.status(201).json({
        ...
        data: { userId: result.rows[0].id, phone },
      });
    } catch (error) {
      next(error);
    }
  ...
  async (req, res, next) => {
    try {
      ...
      const userResult = await query(
        'UPDATE users SET is_verified = TRUE WHERE phone = $1 RETURNING id, name, phone, role, trust_score',
        [phone]
      );
...
const login = [
  ...
  async (req, res, next) => {
    try {
      ...
      const userResult = await query(
        'SELECT id, name, phone, role, trust_score, is_verified, is_active, password_hash FROM users WHERE phone = $1',
        [phone]
      );
