...
const sendNotificationToUser = async ({ userId, title, message, type, groupId = null }) => {
  ...
  if (user?.fcm_token) {
    await sendPushNotification({ fcmToken: user.fcm_token, title, body: message, data: { type, groupId: groupId || '' } });
  }
};

const sendBulkNotifications = async (notifications) => {
  return Promise.allSettled(notifications.map(n => sendNotificationToUser(n)));
};
