...
const contribute = [
  ...
  async (req, res, next) => {
    try {
      ...
      // tc_wallet branch — delegate to WalletService
      if (payment_method === 'tc_wallet') {
        const tcAmount = group.contribution_amount / TC_TO_XAF;
        ...
        if (adminRes.rows[0]) {
          await sendNotificationToUser({
            userId: adminRes.rows[0].user_id, title: 'New Contribution',
            message: `${req.user.name} has contributed ${group.contribution_amount} XAF via TC wallet`,
      ...
      const payment = await processPayment({
        method: payment_method, phone: req.user.phone,
        amount: group.contribution_amount, reference: contribId,
